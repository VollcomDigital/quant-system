...
- `validation.result_consistency` controls strategy-result concentration checks:
  ...
  - outlier-dependency rule is fixed-action: reject result when
    `profit_share_threshold` of positive profit is generated by less than
    `trade_share_threshold` of trades.
